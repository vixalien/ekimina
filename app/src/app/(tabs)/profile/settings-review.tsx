...
import type { SettingsChangeRequest } from "@/api";
...
export default function SettingsReviewScreen(): JSX.Element {
  ...
  const [review, setReview] = useState<SettingsChangeRequest | null>(null);
  ...
  useEffect(() => {
    ...
    dataClient.groups
      .getSettingsChangeReview(activeGroupId, requestId)
      .then(setReview)
      .catch(() => {})
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      const result = await dataClient.groups.signSettingsChange(activeGroupId, requestId, auth.id);
      toast.hide(id);
  ...
  const handleReject = useCallback(
    async (_reason: string) => {
      ...
      setRejecting(true);
      try {
        await dataClient.groups.rejectSettingsChange(activeGroupId, requestId, auth.id);
