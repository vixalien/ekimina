...
function buildRows(summary: ReserveCycleSummary, _currentCycle: number): BreakdownRow[] {
  const rows: BreakdownRow[] = [
    { label: "Contributions in", amount: summary.contributionsIn, colorClass: "text-success", type: "contribution", prefix: "+" },
    { label: "Payout out", amount: summary.payoutOut, colorClass: "text-danger", type: "payout", prefix: "-" },
    { label: "Penalties absorbed", amount: summary.penaltiesAbsorbed, colorClass: "text-success", type: "penalty", prefix: "+" },
    { label: "Loan interest in", amount: summary.loanInterestIn, colorClass: "text-success", type: "loan_repayment", prefix: "+" },
  ];

  if (summary.loanDisbursed != null) {
    rows.push({ label: "Loan disbursed", amount: summary.loanDisbursed, colorClass: "text-danger", type: "loan_disbursement", prefix: "-" });
  }
  if (summary.discretionaryDeposits != null) {
    rows.push({ label: "Discretionary deposits", amount: summary.discretionaryDeposits, colorClass: "text-success", type: "discretionary_deposit", prefix: "+" });
  }
  if (summary.discretionaryWithdrawals != null) {
    rows.push({ label: "Discretionary withdrawals", amount: summary.discretionaryWithdrawals, colorClass: "text-foreground", type: "discretionary_withdrawal", prefix: "-" });
  }
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="pb-36">
          <View className="px-4 pt-4 gap-6">
            ...
            <View className="gap-1">
              <AppText className="text-xs text-muted uppercase tracking-wider">Group reserve</AppText>
              <AppText className="text-4xl font-hero text-foreground">{formatRWF(data.balance)}</AppText>
            </View>
              ...
              {TABS.map((tab) => {
                ...
                return (
                  <PressableFeedback key={tab} animation={false} onPress={() => setHorizon(tab)}>
                    ...
                    >
                      <AppText className={`text-sm font-medium ${isActive ? "text-accent-foreground" : "text-muted"}`}>
                        {tab} cycles
                      </AppText>
                    </View>
                  </PressableFeedback>
            ...
            {/* Breakdown */}
            <AppText className="text-xs text-muted uppercase tracking-wider ml-2">This cycle</AppText>
                    ...
                    <PressableFeedback.Scale>
                      <ListGroup.Item>
                        ...
                        <AppText className={`text-sm font-medium ${row.colorClass}`}>
                          {row.prefix}{formatRWF(row.amount)}
                        </AppText>
            ...
            {data.insight && (
              <Surface variant="secondary" className="px-4 py-3 rounded-xl">
                <View className="flex-row items-start gap-2">
                  <StyledIonicons name="information-circle-outline" size={18} className="text-muted mt-0.5" />
