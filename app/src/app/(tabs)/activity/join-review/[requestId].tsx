...
import { api } from "@/api";
import type { JoinRequestReview } from "@/api/types";
...
export default function JoinReviewScreen(): JSX.Element {
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      const result = await api.groups.signJoinRequest(activeGroupId, requestId, auth.userId);
      toast.hide(id);
  ...
  const handleReject = useCallback(
    async (_reason: string) => {
      ...
      setRejecting(true);
      try {
        await api.groups.rejectJoinRequest(activeGroupId, requestId, auth.userId);
