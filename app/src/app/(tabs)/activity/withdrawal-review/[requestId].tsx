...
import { api } from "@/api";
import type { MemberWithdrawalReview } from "@/api/types";
...
export default function WithdrawalReviewScreen(): JSX.Element {
  ...
  const [review, setReview] = useState<MemberWithdrawalReview | null>(null);
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      const result = await api.groups.signMemberWithdrawal(activeGroupId, requestId, auth.userId);
      toast.hide(id);
  ...
  const handleReject = useCallback(
    async (_reason: string) => {
      ...
      setRejecting(true);
      try {
        await api.groups.rejectMemberWithdrawal(activeGroupId, requestId, auth.userId);
