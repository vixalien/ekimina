...
const currentRoundScreen: ScreenFn = () => ({
  response: end(
    `Round ${group.currentRound} Status
Recipient: Niyonzima David
Due date: 15 Jul 2026
Pool so far: ${group.poolSoFar} USDm

Paid: ${group.paidCount}/${group.totalMembers} members`,
  ),
});
...
Recipient: Niyonzima David
Pool: 0.06 USDm
Paid: ${group.paidCount}/${group.totalMembers}
...
const setRecipientMenu: ScreenFn = (_ctx, key) => {
  ...
  const lines = members.map((m, i) => `${i + 1}. ${m.name}`).join("\n");
  const entries: [string, ScreenFn][] = members.map((_, i) => [
    String(i + 1),
...
export const entryMenu: ScreenFn = (ctx) => {
  const member = members.find((m) => m.phone === ctx.phone);
